...
test("model picker drills into model and effort panes", async ({ page }) => {
  ...
  await page.goto("/");
  await expect(page.locator(".model-picker .picker-trigger")).toContainText("选择模型");
  ...
  await expect(page.locator(".model-picker .picker-trigger")).toContainText("GLM-4.6");
  await expect(page.locator(".model-picker .picker-trigger-effort")).toHaveText("高");
  ...
  await expect(page.locator(".model-picker .picker-trigger-effort")).toHaveText("关闭");
});
...
test("model without reasoning levels omits the effort row", async ({ page }) => {
  ...
  await expect(page.locator(".model-picker .picker-trigger-effort")).toHaveCount(0);
});
