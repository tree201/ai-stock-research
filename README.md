...
公司档案面板：顶栏「公司档案」按钮可展开右侧面板，默认收起；新报告生成时按钮显示红点提示而不自动弹开。面板含四个 Tab——「概览」展示 Yahoo 延迟行情快照（现价、涨跌、52 周区间，原型数据仅供参考）与公司资料/报告计数；「资料」列出该公司已归档文档，并提供研究截止日期与资料 URL 的编辑入口（URL 在下次研究时抓取）；「报告」列出历史报告，点开在弹层中查看全文；「动态」按需加载 Google News 相关新闻。配套 API：`GET /api/company-panel?symbol=&market=`、`GET /api/quote/{symbol}?market=`、`GET /api/news?name=&symbol=`。
