...
import { planChain, toolVerb } from "@/assistant/toolMeta";
...
/** plan 工具的参数文本 → "获取资料 → 分析业务" 计划链摘要。 */
const planSummary = (argsText?: string) => {
  if (!argsText) return "";
  try {
    return planChain(JSON.parse(argsText));
  } catch {
    return "";
  }
};
...
function ToolFallbackTrigger({
  ...
  // 中文动词标签（toolMeta 映射）；plan 工具特化为计划链摘要。
  const chain = toolName === "plan" ? planSummary(argsText) : "";
  const label = isCancelled ? "已取消" : chain ? `制定研究计划：${chain}` : toolVerb(toolName);
...
/** 超长观测截断（fetch_filings 的观测是整页文本，不截断会撑爆 DOM）。 */
const truncate = (text: string) =>
  text.length > RESULT_MAX_CHARS
  ...
  return (
    ...
    >
      ...
      <pre className="aui-tool-fallback-result-content bg-muted/50 text-foreground/90 mt-1 rounded-md p-2.5 text-xs whitespace-pre-wrap">
        {truncate(formatUnknownValue(result, 2))}
      </pre>
    </div>
  );
}
